...
Since the each feature vector has two features, the euclidean distance between two
features ($\bar{x}=(x_1, x_2)$ and $\bar{y}=(y_1, y_2)$) is given by equation
\ref{eq:euclid}, and the Manhattan Distance is given by equation \ref{eq:man}.
...
generalizes better with the Manhattan metric, specially when we consider that the {\bf training error}
was lower for the Manhattan metric, even though it's generalized error was worse.
